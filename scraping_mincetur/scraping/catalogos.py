...
class CatalogosScraper:
    ...
    def fetch_categorias_tree(self) -> List[Dict[str, Any]]:
        ...
        for st in raw_subtipo:
            ...
            if not tipo_id:
                continue
            st_id = st.get("COD_SUBTIPO_CATE") or st.get("COD_SUB_TIPO_CATE") or st.get("atrac_stipo")
            st_nom = (st.get("DES_SUBTIPO_CATE") or st.get("DES_SUB_TIPO_CATE") or st.get("subtipo_categoria") or "").strip()
            if st_id and st_nom:
                subtipos_map.setdefault(tipo_id, []).append({
                    "id_subtipo": int(st_id),
                    "subtipo": st_nom,
                    "id_tipo": int(tipo_id),
                })
    ...
    def fetch_provincias(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for f in features:
                p = f.get("properties", {})
                idprov = str(p.get("CODPROVINCIA") or p.get("CODPROV") or p.get("idprov") or "").strip().zfill(4)
                nombre = (p.get("NOMBRE") or p.get("provincia") or p.get("desprov") or "").strip().upper()
                iddpto = idprov[:2] if len(idprov) >= 2 else str(p.get("CODREGION") or "").strip().zfill(2)
                if idprov and nombre and iddpto and iddpto != "00":
                    provincias.append({
                        "idprov": idprov,
                        "provincia": nombre,
                        "iddpto": iddpto,
    ...
    def fetch_distritos(self) -> List[Dict[str, Any]]:
        """Descarga los 1,800+ Distritos (UBIGEO) desde GeoServer WFS."""
        logger.info("Consultando Distritos (UBIGEO) desde GeoServer WFS...")
        params = {
            "service": "WFS",
            "version": "1.0.0",
            "request": "GetFeature",
        ...
        try:
            ...
            for f in features:
                p = f.get("properties", {})
                ubigeo = str(p.get("CODUBIGEO01") or p.get("ubigeo") or "").strip().zfill(6)
                idprov = str(p.get("CODUBIGEO") or ubigeo[:4] or "").strip().zfill(4)
                nombre = (p.get("NOMBRE") or p.get("distrito") or p.get("desubigeo") or "").strip().upper()
            ...
            distritos.sort(key=lambda x: x["ubigeo"])
            logger.info(f"Se obtuvieron {len(distritos)} distritos (UBIGEO) de GeoServer.")
            return distritos
        except Exception as e:
            logger.warning(f"No se pudieron descargar distritos de GeoServer: {e}")
            return []

    # =========================================================================
    # 4. GUARDAR TODOS LOS CATÁLOGOS EN ORDEN ESTRICTO
    # =========================================================================

    def scrape_and_save_all(self, to_db: bool = True, to_json: bool = False) -> Dict[str, Any]:
        """Ejecuta la extracción de todos los catálogos y los guarda en DB o JSON en orden de dependencia."""
        # 1. Departamentos, Provincias y Distritos
        deptos = self.fetch_departamentos()
        provincias = self.fetch_provincias()
        distritos = self.fetch_distritos()

        # 2. Categorías y Actividades
        categorias = self.fetch_categorias_tree()
        actividades = self.fetch_actividades_tree()

        # Guardar en Base de Datos en orden estricto (Departamentos -> Provincias -> Distritos -> Categorias -> Actividades)
        if to_db:
            from .database import DatabaseManager
            db = DatabaseManager()
            db.save_departamentos(deptos)
            if provincias:
                db.save_provincias(provincias)
            if distritos:
                db.save_distritos(distritos)
            db.save_categorias_tree(categorias)
            db.save_actividades_tree(actividades)

        # Guardar en JSON local si se solicita
        if to_json:
            ...
            if distritos:
                with open(Config.FILE_DISTRITOS, "w", encoding="utf-8") as f:
            ...
            logger.info("✅ Todos los catálogos han sido guardados en data/catalogos/")

        return {
            "departamentos_count": len(deptos),
            "provincias_count": len(provincias),
            "distritos_count": len(distritos),
