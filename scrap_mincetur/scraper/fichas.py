...
class FichasScraper:
    ...
    def scrape_single_ficha(self, cod_ficha: int, session: Optional[requests.Session] = None, force_refresh: bool = False) -> Optional[Dict[str, Any]]:
        """
        Descarga y parsea una ficha individual.
        Si la ficha es válida, enriquece con coordenadas y guarda su JSON individual.
        """
        ...
        if not force_refresh and target_path.exists() and cod_ficha in self.processed_codes:
            try:
    ...
    def scrape_batch(self, codes: List[int], progress_callback: Optional[Any] = None) -> List[Dict[str, Any]]:
        """
        Scrapea un lote de códigos de fichas de forma concurrente con reanudación automática.
        """
        Config.ensure_directories()
        self._load_recursos_map()
        pending_codes = [c for c in codes if c not in self.processed_codes and c not in self.offline_codes]

        logger.info(f"Total códigos a procesar: {len(codes)} (Pendientes: {len(pending_codes)}, Ya procesados: {len(self.processed_codes)}, Offline conocidos: {len(self.offline_codes)})")

        if not pending_codes:
            logger.info("Todos los códigos solicitados ya han sido procesados previamente.")
            return self.load_all_saved_fichas(codes)
        ...
        with ThreadPoolExecutor(max_workers=self.workers) as executor:
            future_to_code = {
                executor.submit(self.scrape_single_ficha, cod): cod
            ...
            for future in as_completed(future_to_code):
                ...
                # Checkpoint periódico cada 30 segundos
